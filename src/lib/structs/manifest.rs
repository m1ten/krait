...
impl KraitScript for KraitManifest {
    fn k_fmt(&self) -> Vec<String> {
        ...
        let mut comments = vec![
            "-- This file is automatically generated by Krait.".to_string(),
        ...
        for (name, versions) in self.packages.as_ref().unwrap() {
            ...
            for (version, packages) in versions {
                ...
                for package in packages {
                    pkgs.push(format!(
                        "    {{ commit = \"{}\", path = \"{}\", timestamp = {}, contents = {{ {} }} }}",
                        package.commit,
                        package.path,
                        package.timestamp,
                        package
...
impl KraitManifest {
    pub fn generate() {
        ...
        let manifest_vector_string = manifest.k_fmt();

        let manifest_string = manifest_vector_string.join("");

        let mut manifest_file = match std::fs::File::create(&manifest_path) {
            Ok(f) => f,
            Err(e) => {
        ...
        let manifest_string = manifest_string.as_bytes();

        bat::PrettyPrinter::new()
            .input_from_bytes(manifest_string)
            .language("lua")
            .line_numbers(true)
            .grid(true)
            .theme("TwoDark")
        ...
        match std::io::Write::write_all(&mut manifest_file, manifest_string) {
            Ok(_) => (),
            Err(e) => {
